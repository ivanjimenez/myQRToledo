...
NSString * myqrouteSp10and$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation myqrouteSp10andObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_myqroutesp10and);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create myqrouteSp10and global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "myqrouteSp10and" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_myqroutesp10and = [[myqrouteSp10andObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_myqroutesp10and];
	
	NSString *_myqroutesp10andNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _myqroutesp10andNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_myqroutesp10and valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:myqrouteSp10and$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.myqroutesp10and.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.myqroutesp10and.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
