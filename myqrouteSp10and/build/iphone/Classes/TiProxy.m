...
//Rare exceptions to indicate a bug in the _myqroutesp10and code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_myqroutesp10and = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_myqroutesp10and]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.myqroutesp10and.module"
